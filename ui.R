  ...
  fluidRow(
    column(12, 
           ...
           fluidRow(column(6, sliderInput(paste0(prefix, "_", "speed"), label = "Speed on all links \\(\\frac{[\\text{km}]}{[\\text{h}]}\\)", min = 10, max = 100, value = 40, step = 1)),
                    column(6, sliderInput(paste0(prefix, "_", "travel_cost"), label = "Travel cost \\(\\frac{[\\text{currency}]}{[\\text{km}]}\\)", min = 0.5, max = 5, value = 1.5, step = 0.1))),
           h5("Travel comfort parameter of the utility function"),
           fluidRow(column(6, sliderInput(paste0(prefix, "_", "beta5"), label = "\\(\\beta_\\text{TT}\\)", min = 0, max = 0.2, value = 0.07, step = 0.001)))
    )   
  )
}
