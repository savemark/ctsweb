...
library(gtools)
library(abind)
setwd("C:/Users/csave/Dropbox/Mathematics/PhD/wei")
source("data.R")
source("city.R")
source("population.R")
source("utility.R")
...
path <- file.path("c:", "Users", "csave", "Dropbox", "Is it OK to ignore land use")
scale <- 40 # km
...
ci <- city.grid(9, scale = scale, mode = "undirected") #ci <- cityA <- cityB <- city.delunay(25*matrix(c(runif(30), runif(30)), 30, 2))
scenario <- "permutation"
linkids <- sample(seq(from = 1, to = getEdgeCount(ci), by = 1), size = 50, replace = FALSE)
alt_speed <- 50
alt_costfactor <- 1.5
