...
#weights[1:17, c(25, 26, 28, 30, 32, 34, 36, 38, 41, 42, 44, 46, 48, 50, 52, 54), 1] <- 0
#weights[1:17, c(25, 26, 28, 30, 32, 34, 36, 38, 41, 42, 44, 46, 48, 50, 52, 54), 2] <- Inf
#weights[2:17, c(25, 26, 28, 30, 32, 34, 36, 38), 1] <- 40
#weights[2:17, c(25, 26, 28, 30, 32, 34, 36, 38), 2] <- 1.5
pop <- population(n, getNodeCount(ci), lowerbound = 2.5*10^5, upperbound = 2*10^6, sdlog=log(1))
...
spillover <- spilloverClosure(eps, getArea(ci), distance = getDistance(ci), b = 1, type = "fromAll")
ec <- economy(ci, pop, weights, utilfunctions, probability, spillover)
#sim0 <- simulate(ec, guess)
sim1 <- simulate(ec, guess, rural.utility = 2)
sim2 <- simulate(ec, guess, rural.utility = 3)
