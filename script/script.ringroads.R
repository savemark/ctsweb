...
# No Ring roads
base_speed[c(9, 10, 12, 14, 16, 18, 20, 22, 25, 26, 28, 30, 32, 34, 36, 38, 41, 42, 44, 46, 48, 50, 52, 54, 57, 58, 59, 60, 61, 62, 63, 64)] <- 0
base_costfactor[c(9, 10, 12, 14, 16, 18, 20, 22, 25, 26, 28, 30, 32, 34, 36, 38, 41, 42, 44, 46, 48, 50, 52, 54, 57, 58, 59, 60, 61, 62, 63, 64)] <- Inf
# 1st ring road
base_speed_ringroad1 <- base_speed
base_costfactor_ringroad1 <- base_costfactor
base_speed_ringroad1[c(9, 10, 12, 14, 16, 18, 20, 22)] <- base_speed_const
base_costfactor_ringroad1[c(9, 10, 12, 14, 16, 18, 20, 22)] <- base_costfactor_const
# 2nd ring road
base_speed_ringroad2 <- base_speed_ringroad1
base_costfactor_ringroad2 <- base_costfactor_ringroad1
base_speed_ringroad2[c(25, 26, 28, 30, 32, 34, 36, 38)] <- base_speed_const
base_costfactor_ringroad2[c(25, 26, 28, 30, 32, 34, 36, 38)] <- base_costfactor_const
# 3rd ring road
base_speed_ringroad3 <- base_speed_ringroad2
base_costfactor_ringroad3 <- base_costfactor_ringroad2
base_speed_ringroad3[c(41, 42, 44, 46, 48, 50, 52, 54)] <- base_speed_const
base_costfactor_ringroad3[c(41, 42, 44, 46, 48, 50, 52, 54)] <- base_costfactor_const
# 4th ring road
base_speed_ringroad4 <- base_speed_ringroad3
base_costfactor_ringroad4 <- base_costfactor_ringroad3
base_speed_ringroad4[c(57, 58, 59, 60, 61, 62, 63, 64)] <- base_speed_const
base_costfactor_ringroad4[c(57, 58, 59, 60, 61, 62, 63, 64)] <- base_costfactor_const
# Weights
...
weights <- abind(speeds, costfactors, along = 3)
...
spillover <- spilloverClosure(eps, getArea(ci), type = "fromAll")
ec <- economy(ci, pop, weights, utilfunctions, probability, spillover)

sim0 <- simulate(ec, guess)
sim1 <- simulate(ec, guess, avg.utility = 3)
